...
def coup_boron():
    ...
    model_name = 'UNet_best_model.epoch256-loss0.00003.hdf5'
    path = 'coup_boron'
    img_height = 128
    img_width = 128
    img_depth = 128
    ...
    if mode == 'predict':
        ...
        for i in range(X_test.shape[0]):
            nor_factor = 0.006121000000000001
            print('predict_test.shape:', predict_test.shape)
            predict = predict_test[i, :, :, :, 0]

            predict[predict < 0] = 0
            Y = np.load(f'../../PHITS_data_processed_for_3DUnet_z_up/1_ratio/GLI_{str(i + pre_data_range[0]).zfill(3)}_GBM_dose_data/data/borondetail_doses.npy')
            ...
            rbe = (np.loadtxt(rbe_path+'/rbe_boron_tissue_128.txt')+np.loadtxt(rbe_path+'/rbe_boron_target_128.txt')).reshape((128, 128, 128))
            ...
            ppm = np.loadtxt(ppm_path+'/b_ppm_128.txt').reshape((128, 128, 128))

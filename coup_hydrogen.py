...
def coup_hydrogen():
    ...
    model_name = 'UNet_best_model.epoch300-loss0.00007.hdf5'
    path = 'coup_hydrogen'
    img_height = 128
    img_width = 128
    img_depth = 128
    ...
    if mode == 'predict':
        ...
        for i in range(X_test.shape[0]):
            nor_factor = 0.003599000000000002
            print('predict_test.shape:', predict_test.shape)
            predict = predict_test[i, :, :, :, 0]

            predict[predict < 0] = 0
            Y = np.load(f'../../PHITS_data_processed_for_3DUnet_z_up/1_ratio/GLI_{str(i + pre_data_range[0]).zfill(3)}_GBM_dose_data/data/hydrogendetail_doses.npy')
            ...
            rbe = np.loadtxt(rbe_path + '/rbe_h_128.txt').reshape((128, 128, 128))
            ppm_path = os.path.join('../../PHITS_data', f'GLI_{patient_number}_GBM_dose_data', '2_PPM_PER')
            ppm = np.loadtxt(ppm_path + '/h_per_128.txt').reshape((128, 128, 128))

...
def coup_photon():
    ...
    model_name = 'UNet_best_model.epoch61-loss0.00019.hdf5'
    path = 'coup_photon'
    img_height = 128
    img_width = 128
    img_depth = 128
    ...
    if mode == 'predict':
        ...
        for i in range(X_test.shape[0]):
            nor_factor = 0.08329000000000002
            print('predict_test.shape:', predict_test.shape)
            predict = predict_test[i, :, :, :, 0]

            predict[predict < 0] = 0
            Y = np.load(f'../../PHITS_data_processed_for_3DUnet_z_up/1_ratio/GLI_{str(i + pre_data_range[0]).zfill(3)}_GBM_dose_data/data/photondetail_doses.npy')
            ...
            rbe = np.loadtxt(rbe_path + '/rbe_x_128.txt').reshape((128, 128, 128))
            ppm_path = os.path.join('../../PHITS_data', f'GLI_{patient_number}_GBM_dose_data', '2_PPM_PER')
            ppm = np.loadtxt(ppm_path + '/p_per_128.txt').reshape((128, 128, 128))
            ...
            coupling_total = np.load(save_path + "/coupling_boron_.npy") + np.load(
                save_path + "/coupling_hydrogen_.npy") + np.load(save_path + "/coupling_nitrogen_.npy") + np.load(
                save_path + "/coupling_photon_.npy")
            monte_total = np.load(save_path + "/monte_boron_.npy") + np.load(
                save_path + "/monte_hydrogen_.npy") + np.load(save_path + "/monte_nitrogen_.npy") + np.load(
                save_path + "/monte_photon_.npy")
            coupling_total = coupling_total.astype(np.float32)
            monte_total = monte_total.astype(np.float32)
            np.save(save_path + "/coupling_" + 'total' + "_.npy", coupling_total)
            np.save(save_path + "/monte_" + 'total' + "_.npy", monte_total)

            def sum_dose(rbe_boron_tissue, rbe_boron_tumor, rbe_hydrogen, rbe_nitrogen, rbe_photon, boron_ppm,
                         hydrogen_per, nitrogen_per, photon_per, boron, hydrogen, nitrogen, photon, boron_err, hydrogen_err,
                         nitrogen_err, photon_err):
                dose = rbe_boron_tissue * boron_ppm * boron + rbe_boron_tumor * boron_ppm * boron + rbe_hydrogen * hydrogen_per * hydrogen + \
                       rbe_nitrogen * nitrogen_per * nitrogen + rbe_photon * photon_per * photon
                err = (rbe_boron_tissue * boron_ppm) ** 2 * (boron * boron_err) ** 2 + \
                      (rbe_boron_tumor * boron_ppm) ** 2 * (boron * boron_err) ** 2 + \
                      (rbe_hydrogen * hydrogen_per) ** 2 * (hydrogen * hydrogen_err) ** 2 + \
                      (rbe_nitrogen * nitrogen_per) ** 2 * (nitrogen * nitrogen_err) ** 2 + \
                      (rbe_photon * photon_per) ** 2 * (photon * photon_err) ** 2
                standard_error = np.sqrt(err)
                # 防止dose为0时被除
                relative_err = np.divide(standard_error, dose, out=np.zeros_like(standard_error), where=dose != 0)
                return relative_err

            rbe_b_tissue = np.loadtxt(rbe_path + '/rbe_boron_tissue_128.txt').reshape((128, 128, 128))
            rbe_b_tumor =  np.loadtxt(rbe_path + '/rbe_boron_target_128.txt').reshape((128, 128, 128))
            skin = np.loadtxt(rbe_path + '/skin_128.txt').reshape((128, 128, 128))
            rbe_b_tissue[skin == 1] = 2.5
            ppm_b = np.loadtxt(ppm_path + '/b_ppm_128.txt').reshape((128, 128, 128))
            dose_b = np.load(save_path+'/borondetail_doses.npy')
            err_b = np.load(save_path+'/boron_err_128.npy')

            rbe_h = np.loadtxt(rbe_path + '/rbe_h_128.txt').reshape((128, 128, 128))
            ppm_h = np.loadtxt(ppm_path + '/h_per_128.txt').reshape((128, 128, 128))
            dose_h = np.load(save_path + '/hydrogendetail_doses.npy')
            err_h = np.load(save_path + '/hydrogen_err_128.npy')

            rbe_n = np.loadtxt(rbe_path + '/rbe_n_128.txt').reshape((128, 128, 128))
            ppm_n = np.loadtxt(ppm_path + '/n_per_128.txt').reshape((128, 128, 128))
            dose_n = np.load(save_path + '/nitrogendetail_doses.npy')
            err_n = np.load(save_path + '/nitrogen_err_128.npy')

            rbe_x = np.loadtxt(rbe_path + '/rbe_x_128.txt').reshape((128, 128, 128))
            ppm_x = np.loadtxt(ppm_path + '/p_per_128.txt').reshape((128, 128, 128))
            dose_x = np.load(save_path + '/photondetail_doses.npy')
            err_x = np.load(save_path + '/photon_err_128.npy')

            monte_total_err = sum_dose(rbe_b_tissue, rbe_b_tumor, rbe_h, rbe_n, rbe_x, \
                                       ppm_b, ppm_h, ppm_n, ppm_x, \
                                       dose_b, dose_h, dose_n, dose_x, \
                                       err_b, err_h, err_n, err_x)
            np.save(save_path + '/total_err_128.npy', monte_total_err)
        # endregion
    # endregion
